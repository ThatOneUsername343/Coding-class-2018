...
public class ChibiBehav : MonoBehaviour
{
    ...
    public bool notUprightTouchingGround;
    ...
    void Update()
    {
        ...
        if (touchingGround == true)
        {
            ...
            if(canJump == true)
            {
                spriteRenderer.sprite = sticker_1;
            }
            ...
            //Check if it's the right time for the chibi to hop
            if (time >= HopTime)
            {
                Hop();
                SetRandomTime();
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
        if (collision.collider.tag == "Ground")
        {
            if (transform.rotation.eulerAngles.x <= 0 && transform.rotation.eulerAngles.z <= 0 && transform.rotation.eulerAngles.y <= 0 || transform.rotation.eulerAngles.x <= 0 && transform.rotation.eulerAngles.z <= 0 && transform.rotation.eulerAngles.y <= 180)
            {
                ...
                canHop = true;
                canJump = true;
                notUprightTouchingGround = false;
            }

            else
            {
                notUprightTouchingGround = true;
            }
        }
    }
    ...
    void OnMouseExit()
    {
        if (touchingGround == true && canJump == true)
        {
            //if (transform.rotation.eulerAngles.x <= 0 && transform.rotation.eulerAngles.z <= 0 && transform.rotation.eulerAngles.y <= 0 || transform.rotation.eulerAngles.x <= 0 && transform.rotation.eulerAngles.z <= 0 && transform.rotation.eulerAngles.y <= 180)
            //{
            //ChangeTheSprite();
